...
DREAMER_STRATEGY = "dreamer_strategy"  # Dreamer strategy
...
def run_full_circuit(observable, backend_options, full_circuit):
    try:
        ...
        print(f"Exact expectation value: {exact_expval}")
...
class CircuitCuttingBuilder:
    ...
    def set_scenario_label(self, scenario_label):
        ...
        return self
    def set_dreamer_strategy(self, dreamer_strategy):
        self.dreamer_strategy = dreamer_strategy
        return self
...
class CircuitCutting(Motif):
    ...
    def get_sub_circuit_observables(self, circuit, observable, num_samples=10):
        """
    ...
    def run_circuit_cutting(self, circuit, observable, circuit_cutting_qiskit_options):
        ...
        for label, subexperiment_list in subexperiments.items():
            ...
            task_future = self.executor.submit_quantum_task(quantum_task)
            quantum_task_futures[label]=task_future

        quantum_tasks_submission_end_time = time.time()
        quantum_tasks_submission_time_secs = quantum_tasks_submission_end_time - quantum_tasks_submission_start_time
        ...
        quantum_tasks_results_start_time = time.time()
        quantum_results = {}
        for label, task_future in quantum_task_futures.items():
            result = self.executor.get_results([task_future])[0]
        ...
        self.logger.info(f"Reconstructed expectation value: {np.real(np.round(reconstructed_expval, 8))}")
        end_time = time.time()
        total_runtime_secs = end_time - start_time
    ...
    def run_full_circuit_simulation(self, circuit, observable, full_circuit_qiskit_options):
        ...
        if "mpi" not in full_circuit_qiskit_options or full_circuit_qiskit_options["mpi"] == False:
            ...
            # QuantumTask now expects the function as a keyword argument 'func', with positional 'args' or 'kwargs'
            quantum_task = QuantumTask(
                circuit=run_full_circuit,
                args=[observable, full_circuit_qiskit_options, full_circuit_transpilation]
            )

            full_circuit_task = self.executor.submit_quantum_task(
                quantum_task
            )
